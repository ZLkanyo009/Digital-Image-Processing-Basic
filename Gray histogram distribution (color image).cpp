...
int main()
{
	...
	{
		//三个channel取平均分析灰度
		line(histoImage, Point(50,-1000),
			Point(50,500), Scalar(0, 255, 0));
		line(histoImage, Point((i - 1)*bin_w, histo_h - (cvRound(blue_histo.at<float>(i - 1)) + cvRound(green_histo.at<float>(i - 1)) + cvRound(red_histo.at<float>(i - 1))) / 3),
			Point((i)*bin_w, histo_h - (cvRound(blue_histo.at<float>(i)) + cvRound(green_histo.at<float>(i)) + cvRound(red_histo.at<float>(i))) / 3), Scalar(255, 255, 255), 2, CV_AA);
	}
